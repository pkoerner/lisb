...
(deftest machine-test
  (testing "empty machine"
    ...
    (is (= "MACHINE Empty\nEND"
           (get-machine-from-ast (b->ast (b->lisb "MACHINE Empty\nEND")))))))
    ...
    (testing "constants"
      (let [bmachine (slurp (clojure.java.io/resource "machines/Constant.mch"))]
        (is (= (get-machine-from-ast (parse-b-machine bmachine)) ; formats bmachine
               (get-machine-from-ast (b->ast (b->lisb bmachine))))))))
...
(deftest sets-test
  (testing "sets"
    (let [empty-set "{}"
          ...
          set-enum2-alt "{F,E}"]
      (is (= empty-set
             (get-machine-from-ast (b->ast (b-formula->lisb empty-set)))))
      (is (= set-enum1
             (get-machine-from-ast (b->ast (b-formula->lisb set-enum1)))))
      (is (let [machine-set-enum2 (get-machine-from-ast (b->ast (b-formula->lisb set-enum2-alt)))] ; order doesn't matter
...
(deftest equality-predicates-test
  (testing "equality-predicates"
    (let [equal "TRUE = FALSE"
          unequal "TRUE /= FALSE"]
      (is (= equal (print-ast (b->ast (b-predicate->lisb equal))))))))