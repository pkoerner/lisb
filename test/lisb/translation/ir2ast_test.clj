...
(deftest struct-test
  (testing "structs"
    (are [b ir] (= b (ast->b (ir->ast ir)))
                  ...
                  "rec(n:1)" (b (rec :n 1))
                  "rec(n:1,b:TRUE)" (b (rec :n 1, :b true))
                  "R'n" (b (get :R :n)))))
...
(deftest sets-test
  (testing "sets"
    ...
    (are [b ir] (= b (ast->b (ir->ast ir)))
                  ...
                  "{E}<<:{G}" (b (strict-subset? #{:E} #{:G}))
                  "{E}/<<:{G}" (b (not (strict-subset? #{:E} #{:G})))
                  "{G}<:{E}" (b (superset? #{:E} #{:G}))
                  "{G}/<:{E}" (b (not (superset? #{:E} #{:G})))
                  "{G}<<:{E}" (b (strict-superset? #{:E} #{:G}))
                  "{G}/<<:{E}" (b (not (strict-superset? #{:E} #{:G}))))))
...
(deftest logical-predicates-test
  (testing "logical-predicates"
    (are [b ir] (= b (ast->b (ir->ast ir)))
                ...
                "!x.(x:NAT => 0<=x)" (b (for-all [:x nat-set] (<= 0 :x)))
                "!x,y.(x:NAT & y:NAT => 0<=x+y)" (b (for-all [:x :y] (and (member? :x nat-set) (member? :y nat-set)) (<= 0 (+ :x :y))))
                "!x,y.(x:NAT & y:NAT => 0<=x+y)" (b (for-all [:x nat-set :y nat-set] (<= 0 (+ :x :y))))
