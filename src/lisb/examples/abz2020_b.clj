(ns lisb.examples.abz2020-b
  (:require [lisb.translation.util :as util]
            ...
            [lisb.translation.eventb.ir2eventb :refer [ir->prob extract-partitions]]
...
(defn replace-args [args op]
  (let [replacement (into {} (map vector (:args op)args))]
    (s/transform [:body (s/walker replacement)] replacement op)))

(comment
  (clojure.pprint/pp)

  (->> (b2eventb/inline-all-op-calls PitmanController Sensors)
  ...
  (->> (b2eventb/includes->inline PitmanController Sensors) b2eventb/extract-context
       ir->prob
       .getAxioms
      (map #(.getPredicate %)))

  (clojure.pprint/pprint BlinkLamps)
  (-> "../../models/abz2020-models-master/LightModel/PitmanController_v6.mch"
      slurp
      util/b->lisb
      clojure.pprint/pprint)
