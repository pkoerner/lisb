(ns lisb.translations.blojure-translation
  (:require [clojure.math.combinatorics :refer [combinations]])
  (:require [clojure.walk :refer [walk]])
  (:require [lisb.frontends.blojure :refer [node bset-enum btuple]])
...
(def to-ast-map {:keyword identifier
                 :less less-node
                 ...
                 :card card-node
                 :set enumerated-set-node ; KABOOM?
...
(defn to-ast-inner [data]
  (cond
    (#{:boolean :keyword :number :string} (:tag data)) (apply (to-ast-map (:tag data)) (:children data))
    :otherwise (let [processed-args (to-ast-wrapped (:children data))]
...
(defn prepare-maps [data]
  (if (= (:tag data) :map)
    (apply bset-enum (map (fn [[k v]] (btuple k v)) (:children data)))
    data))

(defn to-ast-wrapped [datav]
  (walk (comp to-ast-inner make-binary prepare-maps) identity datav))
